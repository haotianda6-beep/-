...
from app.v2_support import TERMINAL, execution_status, lots_from_qty
...
class GoldV2Executor:
    ...
    async def cancel_active_order(self, reason: str) -> None:
        if not self.active_order or self.active_order.status in TERMINAL:
            return
        try:
    ...
    def execution_plan_status(self) -> ExecutionPlanStatus:
        return execution_status(
            self.settings, self.active_order, self.runtime.open_pair, self.entry_hedge_side, self._target_exit_spread
        )
    ...
    async def _maybe_place_exit(self) -> None:
        ...
        if not pair or not quote or not mt4_quote:
            return
        target = self._target_exit_spread(pair)
    ...
    def _handle_hedge_report(self, report) -> None:
        ...
        self.runtime.open_pair = OpenPair(
            direction=self.entry_direction,
            quantity_oz=self.active_order.executed_qty,
            binance_entry_price=self.active_order.avg_price,
            mt4_entry_price=report.fill_price,
            binance_order_id=self.active_order.order_id,
            mt4_ticket=report.ticket,
            mt4_tickets=[report.ticket] if report.ticket else [],
            base_edge=edge,
        )
    ...
    def _target_exit_spread(self, pair: OpenPair) -> Decimal:
        base = pair.base_edge if pair.base_edge is not None else abs(pair.binance_entry_price - pair.mt4_entry_price)
        return max(Decimal("0"), base - self.settings.close_profit_usd_per_oz)
