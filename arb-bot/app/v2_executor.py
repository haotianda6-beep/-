...
class GoldV2Executor(V2AddMixin, V2CommonMixin):
    ...
    async def _check_entry_order(self, plan_status: dict[str, Any]) -> None:
        ...
        if order.executed_qty > 0:
            ...
            if utc_now_ms() - self.order_created_ms > max(self.settings.min_order_live_ms, self.settings.max_order_age_ms):
                await self._cancel_partial_entry_for_requote(order, plan_status)
                return
    ...
    async def _cancel_partial_entry_for_requote(self, order: OrderUpdate, plan_status: dict[str, Any]) -> None:
        try:
            canceled = await self.binance.cancel_order(order.order_id)
        except Exception as exc:  # noqa: BLE001
            self.runtime.last_error = f"币安开仓部分成交单撤剩余失败，继续重试：{str(exc)[:160]}"
            self.storage.record_event(
                "v2_entry_partial_requote_cancel_failed",
                {"error": str(exc)[:160], "order": order.model_dump(mode="json")},
            )
            return
        checked = canceled or order
        self.storage.record_event(
            "v2_entry_partial_requote_cancelled",
            {"order": checked.model_dump(mode="json"), "reason": "部分成交超过最长等待，撤剩余后重挂。"},
        )
        await self._replace_remaining_entry_order(checked, plan_status)
