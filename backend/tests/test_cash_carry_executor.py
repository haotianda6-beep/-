...
def test_shadow_probe_requires_same_exchange_shadow_samples(tmp_path) -> None:
    state = tmp_path / "state.json"
    state.write_text(_shadow_state(5, ExchangeName.BITGET), encoding="utf-8")
    executor = CashCarryExecutor(state)
    settings = BotSettings(order_notional_usdt=Decimal("300"), cash_carry_recovery_probe_notional_usdt=Decimal("100"))
    item = _opportunity(net="2.0", basis="0.7", exchange=ExchangeName.GATE).model_copy(
        update={"blocked_reasons": ["合约溢价未达 0.8%", "V3冷启动净利预估 2.0000U < 冷启动安全垫 3.0000U"]}
    )
...
def _shadow_state(count: int, exchange: ExchangeName) -> str:
    rows = []
    for index in range(count):
        rows.append(
            '{"opened_at":"2026-06-28T00:00:00+00:00","exchange":"'
            + exchange.value
            + '","symbol":"SHADOW'
            + str(index)
