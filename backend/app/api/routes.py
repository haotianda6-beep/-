...
@router.get("/alpha-alert/opportunities")
async def get_alpha_alert_opportunities():
    _, runtime = await _runtime()
...
def _loading_snapshot() -> RealtimeSnapshot:
    ...
    return RealtimeSnapshot(
        ...
        alpha_alert_opportunities=[],
        alpha_alert_candidates=[],
...
def _lightweight_snapshot() -> RealtimeSnapshot:
    ...
    risk_events = [
        RiskEvent(
            ...
            severity="info",
            title="主控台轻量实时模式",
            detail="主控台只保留正向期现实时数据、参数和 API 管理；已关闭无关扫描，避免页面进入受外部接口影响。",
            action="黄金价差套利请进入 /xau-arb/ 独立执行器；MT4 与五所价差扫描需要时单独开启。",
            created_at=now,
        )
    ]
    risk_events.extend(
