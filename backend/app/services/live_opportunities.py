...
class LiveOpportunityScanner:
    ...
    def scan(self, settings: BotSettings) -> LiveOpportunityScan:
        ...
        rows: list[tuple[Opportunity, list[str]]] = []
        for long_exchange in ExchangeName:
            for short_exchange in ExchangeName:
                if long_exchange != short_exchange and not self._is_blocked(long_exchange, short_exchange, settings):
                    rows.extend(self._pair_opportunities(by_exchange[long_exchange], by_exchange[short_exchange], settings))
        ranked_base = sorted(rows, key=lambda item: (len(item[1]), -item[0].estimated_net_profit))[:25]
        checked = [self._validate_opportunity(item, reasons, by_exchange, settings) for item, reasons in ranked_base]
        ...
        candidates = [self._candidate_from(item, reasons) for item, reasons in ranked]
    ...
    def _extract_swaps(self, markets: dict[str, Any]) -> dict[str, SwapMarket]:
        ...
        for market in markets.values():
            if not market.get("swap") or market.get("quote") != "USDT":
                continue
            if market.get("settle") not in (None, "USDT") or market.get("active") is False:
                continue
            symbol = normalized_market_symbol(market)
            taker = decimal_from(market.get("taker"), "0")
            swaps[symbol] = SwapMarket(
                symbol=symbol,
    ...
    def _extract_spots(self, markets: dict[str, Any]) -> dict[str, MarketAsset]:
        ...
        for market in markets.values():
            if market.get("spot") and market.get("quote") == "USDT" and market.get("active") is not False:
                spots[normalized_market_symbol(market)] = asset_from_market(market)
        return spots

    def _fetch_tickers(self, exchange, swaps: dict[str, SwapMarket], exchange_name: ExchangeName, issues: list[str]) -> dict[str, dict[str, Any]]:
    ...
    def _fetch_funding_rates(self, exchange, swaps: dict[str, SwapMarket], exchange_name: ExchangeName, issues: list[str]) -> dict[str, Decimal]:
    ...
    def _build_opportunity(
        ...
        estimated_net = settings.order_notional_usdt * spread_pct / Decimal("100") - fees + funding_net
    ...
    def _candidate_from(self, opportunity: Opportunity, reasons: list[str]) -> OpportunityCandidate:
        return OpportunityCandidate(**opportunity.model_dump(), blocked_reasons=reasons or ["满足当前开仓条件，已进入可开仓机会表"])
    ...
    def _depth_ok(
        ...
        try:
            return has_depth(long_data.swap_exchange, long_market.ccxt_symbol, "long", long_price, required, settings.max_slippage_pct) and has_depth(
                short_data.swap_exchange,
                short_market.ccxt_symbol,
                "short",
                short_price,
                required,
