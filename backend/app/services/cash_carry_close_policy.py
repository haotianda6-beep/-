...
def cash_carry_close_decision(
    ...
    *,
    ...
    if has_live_net and settings.stop_loss_usdt > 0 and current_net_profit <= -settings.stop_loss_usdt:
        return CashCarryCloseDecision(True, f"固定U止损达到 {current_net_profit} USDT")
    ...
    return CashCarryCloseDecision(False, f"基差已收敛但当前亏损 {current_net_profit} USDT，未触发固定止损，不主动亏损平仓")
