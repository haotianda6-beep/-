...
class DeepSeekMonitor:
    ...
    def insight(
        ...
        signature = self._signature(balances, positions, risk_events, extra)
        with self._lock:
            ...
            if not self._refreshing:
                ...
                threading.Thread(
                    ...
                    args=(list(balances), list(positions), list(risk_events), extra, signature),
                    daemon=True,
    ...
    def _refresh(
        ...
        now = datetime.now(timezone.utc)
        try:
            insight = self._call_deepseek(balances, positions, risk_events, extra, now)
    ...
    def _call_deepseek(
        ...
        payload = {
            "model": model,
            "messages": [
                {"role": "system", "content": "你是期现套利系统的风控监控。只输出风险观察、异常、建议；不要直接要求自动下单。"},
                {"role": "user", "content": self._build_prompt(balances, positions, risk_events, extra)},
            ],
            "temperature": 0.2,
            "max_tokens": 700,
            "stream": False,
